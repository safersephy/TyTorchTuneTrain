...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=0, dropout=0.0, maxpool=True):
...
class cnn(nn.Module):
    def __init__(self, config: dict):
        ...
        for i, blockconfig in enumerate(config["conv_blocks"]):
            in_ch = self.input_size[1] if i == 0 else config["conv_blocks"][i-1]["filters"]
            self.convolutions.append(
                ConvBlock(
                    in_channels=in_ch,
                    out_channels=blockconfig["filters"],
                    kernel_size=blockconfig.get("kernel_size", 3),
                    padding=blockconfig.get("padding", 1),
                    dropout=blockconfig.get("dropout", 0.0),
                    maxpool=blockconfig.get("maxpool", True)
                )
            )
        ...
        for i, blockconfig in enumerate(config["linear_blocks"]):
            in_features = config["conv_blocks"][-1]["filters"] if i == 0 else config["linear_blocks"][i-1]["out_features"]
